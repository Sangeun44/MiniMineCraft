...
ShaderProgram::ShaderProgram(OpenGLContext *context)
    : vertShader(), fragShader(), prog(), unifSampler2D(-1), unifTime(-1),
      attrPos(-1), attrNor(-1), attrCol(-1), attrUV(-1), attrBlock(-1), attrData(-1),
      unifModel(-1), unifModelInvTr(-1), unifViewProj(-1), unifColor(-1),
      unifEyePos(-1), drawOpaque(true),
      context(context)
{}
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    attrBlock = context->glGetAttribLocation(prog, "vs_Block");
    attrData = context->glGetAttribLocation(prog, "vs_Data");
...
void ShaderProgram::setEyePos(glm::vec3 p) {
    useMe();
    if (unifEyePos != -1) {
        context->glUniform4fv(unifEyePos, 1, &p[0]);
    }
...
void ShaderProgram::draw(Drawable &d)
{
    ...
    if (attrBlock != -1 && d.bindBlocks()) {
        context->glEnableVertexAttribArray(attrBlock);
        context->glVertexAttribPointer(attrBlock, 1, GL_FLOAT, false, 0, NULL);
    }

    if(drawOpaque) {
        if (attrPos != -1 && attrNor != -1 && attrUV != -1 && d.bindData()) {
            context->glEnableVertexAttribArray(attrPos);
            context->glVertexAttribPointer(attrPos, 4, GL_FLOAT, false, (2 * sizeof(glm::vec4)) + sizeof(glm::vec2) + sizeof(GL_FLOAT), BUFFER_OFFSET(0));

            context->glEnableVertexAttribArray(attrNor);
            context->glVertexAttribPointer(attrNor, 4, GL_FLOAT, false, (2 * sizeof(glm::vec4)) + sizeof(glm::vec2) + sizeof(GL_FLOAT), BUFFER_OFFSET(sizeof(float) * 4));

            context->glEnableVertexAttribArray(attrUV);
            context->glVertexAttribPointer(attrUV, 2, GL_FLOAT, false, (2 * sizeof(glm::vec4)) + sizeof(glm::vec2) + sizeof(GL_FLOAT), BUFFER_OFFSET(sizeof(float) * 8));

            context->glEnableVertexAttribArray(attrBlock);
            context->glVertexAttribPointer(attrBlock, 1, GL_FLOAT, false, (2 * sizeof(glm::vec4)) + sizeof(glm::vec2) + sizeof(GL_FLOAT), BUFFER_OFFSET(sizeof(float) * 10));
        }
        ...
        if (attrPos != -1 && attrNor != -1 && attrUV != -1 && d.bindDataTrans()) {
            context->glEnableVertexAttribArray(attrPos);
            context->glVertexAttribPointer(attrPos, 4, GL_FLOAT, false, (2 * sizeof(glm::vec4)) + sizeof(glm::vec2) + sizeof(GL_FLOAT), 0);
    ...
    if (attrBlock != -1) context->glDisableVertexAttribArray(attrBlock);
